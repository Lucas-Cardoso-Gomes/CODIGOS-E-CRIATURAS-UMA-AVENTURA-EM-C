...
void mostrarInformacoesCombate(Jogador* jogador) {
    ...
    if (jogador->escudoAtivo) {
        printf(" \n| Escudo ativo (Duração restante: %d turnos)\n", jogador->duracaoEscudo);
    } else {
        printf("\n");
    }
}
...
void executarCura(Jogador* jogador) {
    if (jogador->MP >= CUSTO_CURA) {
        printf("\nVocê conjura a Cura! Você recupera 1 ponto de vida.\n");
        jogador->HP++;
...
void campanha3(Sessao* inicioSessao)/*Caverda dos goblins*/ {
    ...
    Sessao* sessao10 = criarSessao("\nVocê, finalmente, derrota todos os Golgorgs, mas a caverna começa a desmoronar. É hora de escapar o mais rápido possível.");
