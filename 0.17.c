...
Sessao* criarSessao(const char descricao[500]) {
    ...
    if (novaSessao == NULL) {
        printf("\nErro ao alocar mem¢ria para iniciar a sessÆo.\n");
        exit(EXIT_FAILURE);
    }
...
void mostrarInformacoesCombate(Jogador* jogador) {
    ...
    if (jogador->escudoAtivo) {
        printf(" \n| Escudo ativo (Dura‡Æo restante: %d turnos)\n", jogador->duracaoEscudo);
    } else {
        printf("\n");
    }
}
...
void executarAtaque(Jogador* jogador, Sessao* sessaoAtual) {
    ...
    if ((resultadoDado-= sessaoAtual->mod_inimigo) >= 14) {
        printf("\nVocˆ derrotou o inimigo e avan‡a com cuidado.\n");
        ...
        if (sessaoAtual->mod_inimigo > 1){
            printf("\nO Inimigo ‚ poderoso, vocˆ ir  precisar de mais precisÆo em seus ataques!\n");
        }
    } else {
        printf("\nVocˆ escorregou e o inimigo te ataca! Vocˆ perde 1 HP.\n");
        dano(jogador, 1);
    ...
    if (jogador->HP <= 0) {
        printf("\nVocˆ perdeu todas as vidas. Fim do jogo.\n");
        exit(EXIT_SUCCESS);
    }
    ...
    if (sessaoAtual->inimigo == 1) {
        ...
        if (resultadoDadoInimigo >= 19) {
            printf("\nO inimigo acertou vocˆ e causou 2 de dano!\n");
            dano(jogador, 2);
        } else if (resultadoDadoInimigo >= 12) {
            printf("\nO inimigo acertou vocˆ e causou 1 de dano.\n");
            dano(jogador, 1);
        } else if (resultadoDadoInimigo <= 2) {
            printf("\nO inimigo escorregou e vocˆ contra-atacou!\n");
            sessaoAtual->inimigo = 0;
        } else {
...
void executarBolaDeFogo(Jogador* jogador, Sessao* sessaoAtual) {
    ...
    if (jogador->MP >= CUSTO_BOLA_DE_FOGO) {
        ...
        if (resultadoDado > 10) {
            printf("\nVocˆ acertou o inimigo e o derrotou.\n");
            sessaoAtual->inimigo = 0;
        } else {
        ...
        printf("\nVocˆ nÆo possui PM suficiente para conjurar a Bola de Fogo.\n");
    }
    ...
    if (sessaoAtual->inimigo == 1) {
        ...
        if (resultadoDadoInimigo >= 19) {
            ...
            printf("\nO inimigo escorregou e vocÒ contra-atacou!\n");
            sessaoAtual->inimigo = 0;
        } else {
            printf("\nO inimigo errou o ataque..\n");
        }
...
void executarEscudo(Jogador* jogador) {
    ...
    if (jogador->MP >= CUSTO_ESCUDO) {
        if (jogador->escudoAtivo) {
            printf("\nVocˆ j  conjurou o Escudo. Escolha outra a‡Æo.\n");
        } else {
            printf("\nVocˆ conjura o Escudo! Sua defesa est  melhorada.\n");
            jogador->escudoAtivo = 1;
            jogador->duracaoEscudo = 3;
            jogador->MP -= CUSTO_ESCUDO;
        }
    } else {
        printf("\nVocˆ nÆo possui PM suficiente para conjurar o Escudo.\n");
    }
}
...
void executarCura(Jogador* jogador) {
    ...
    if (jogador->MP >= CUSTO_CURA) {
        printf("\nVocˆ conjura a Cura! e recupera 1 ponto de vida.\n");
        jogador->HP++;
        jogador->MP -= CUSTO_CURA;
    } else {
        printf("\nVocˆ nÆo possui PM suficiente para conjurar a Cura.\n");
    }
}
...
void executarCombate(Jogador* jogador, Sessao* sessaoAtual) {
    ...
    {
        printf("\nEles sÆo muitos, continue lutando por sua vida!\n");
...
void executarArmadilha(Jogador* jogador, Sessao* sessaoAtual) {
    ...
    printf("\nVocÒ se depara com uma armadilha!\n");
        ...
        switch (opcao)
    {
        case 1:
            executarEscudo(jogador);
            printf("\nVocˆ conjura o escudo e tenta passar pela armadilha...\n");
            break;
        case 2:
            executarCura(jogador);
            printf("\nVocˆ conjura o cura e tenta passar pela armadilha...\n");
            break;
        case 3:
            printf("\nVocˆ tenta passar pela armadilha...\n");
            break;
    }
    ...
    if (resultadoDado >= 15) {
        printf("\nVocˆ conseguiu desarmar a armadilha e sair ileso!\n");
        sessaoAtual->armadilha=0;
    } else if (resultadoDado >= 7) {
        printf("\nVocˆ passou pela armadilha, mas sofreu um pequeno ferimento. Perdeu 1 ponto de vida.\n");
        dano(jogador, 1);
        sessaoAtual->armadilha=0;
    } else if (resultadoDado >= 2) {
        printf("\nVocˆ est  preso na armadilha e sofreu um pequeno ferimento. Perdeu 1 ponto de vida.\n");
        dano(jogador, 1);
    } else {
        printf("\nVocˆ est  preso na armadilha e sofreu um ferimento grave. Perdeu 2 ponto de vida.\n");
        dano(jogador, 2);
    }
    if (jogador->HP <= 0) {
            printf("\nVocˆ perdeu todas as vidas. Fim do jogo.\n");
...
void executarSuprimentos(Jogador* jogador, Sessao* sessaoAtual) {
    ...
    printf("\nVocˆ encontrou suprimentos durante sua jornada, que tenham boa utilidade!\n");

    int resultadoDado = rolarD20();
    printf("\nSorte no dado %d\n", resultadoDado);

    if (resultadoDado >= 15) {
        printf("\nVocˆ recebe um Buff de +3 HP e 10 MP!\n");
        jogador->HP += 3;
        jogador->MP += 10;
    } else if (resultadoDado >= 7) {
        printf("\nVocˆ recebe um Buff de +2 HP e 6 MP!\n");
        jogador->HP += 2;
        jogador->MP += 6;
    } else if (resultadoDado >= 2) {
        printf("\nVocˆ recebe um Buff de +1 HP e 3 MP!\n");
        jogador->HP += 1;
        jogador->MP += 3;
    } else if (resultadoDado == 2) {
        printf("\nVocˆ recebe um Buff de +2 MP!\n");
        jogador->MP += 2;
    }
    else {
        printf("\nOs suprimentos estÆo estragados, nada poder  ser usado.\n");
    }
...
void jogarJogo(Sessao* inicioSessao, Jogador* jogador) {
    ...
    printf("\nParab‚ns! Vocˆ explorou a caverna com sucesso!\n");
...
void campanha1(Sessao* inicioSessao)/*Main Quest*/ {
    Sessao* sessao1 = criarSessao("\nVocˆ est  a bordo de um carr‡a, sendo levado para sua execu‡Æo em Helgen.");
    ...
    sessao16->inimigo = 1;
    Sessao* sessao17 = criarSessao("\nFinalmente, a Luz do dia nunca foi tÆo boa, vocˆs fogem s  em salvo!");
...
void campanha3(Sessao* inicioSessao)/*Caverna dos Golgorgs*/ {
    Sessao* sessao1 = criarSessao("\nVocˆ adentra a Caverna dos Golgorgs, sua missÆo ‚ massacrar esses pequenos monstros.");
    Sessao* sessao2 = criarSessao("\nOs Golgorgs espreitam vocˆ de longe, mas nÆo tentam aproximar, parecem tramar algo...");
    sessao2->armadilha = 1;
    Sessao* sessao3 = criarSessao("\nVocˆ consegue se livrar da armadilha e segue a caverna.");
    Sessao* sessao4 = criarSessao("\nVocˆ encontra uma encruzilhada. Um grupo de Golgorgs surge de uma passagem lateral.");
    sessao4->inimigo = 1;
    Sessao* sessao5 = criarSessao("\nAp¢s derrotar os Golgorgs, vocˆ encontra uma sala cheia de tesouros pequenos e reluzentes e encontra alguns suprimentos que parecem ter sido roubados de outros aventureiros.");
    sessao5->suprimentos = 1;
    Sessao* sessao6 = criarSessao("\nContinuando sua explora‡Æo, vocˆ se depara com um Golgorgs no fim de uma sala, ao tentar se aproximar vocˆ ‚ surpreendido.");
    sessao6->armadilha = 1;
    Sessao* sessao7 = criarSessao("\n\tO Golgorgs no fim sala ‚ o l¡der Golgorgs desta caverna, ele tenta correr para outra passagem na caverna e vocˆ o segue...");
    sessao7->armadilha = 1;
    ...
    Sessao* sessao10 = criarSessao("\nVocˆ, finalmente, derrota todos os Golgorgs, mas a caverna come‡a a desmoronar.  hora de correr o mais r pido poss¡vel.");
    sessao10->armadilha = 1;
    Sessao* sessao11 = criarSessao("\nVocˆ escapa da Caverna dos Golgorgs a tempo. Sua missÆo de elimina‡Æo foi conclu¡da com sucesso!.");
...
void help(){
    ...
    printf("\n3. Suprimentos\n\t Vocˆ utiliza de suprimentos dispon¡veis naquele momento, a quantidade de HP e MP recuperada ir  depender da sua sorte!\n");
...
int main() {
    ...
    do{
        ...
        printf(R"EOF(
Escolha uma op‡Æo abaixo para iniciar uma jornada:
...
4. Covil dos Orcs - Jornada para aventureiros que nÆo temem a morte
5. Ajuda
6. Sair)EOF");
printf("\n");
        scanf("\n%d", &gamemode);
